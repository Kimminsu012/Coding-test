...
//      변수는 단하나의 값만 저장 할 수 있지만
//      배열은 다수의 값을 저장할 수 있다.
//      많은 데이터를 저장하고 관리하기에 가장 쉬운 방법이다.

// var arr1 = []; // 메모리 주소를 arr1의 이름을 가진 참조변수에 넣는다.
...
// // 1번째 - 특정위치를 선택, 2번째 - 특정위치 선택 후 삭제할 갯수, 3번째 - 추가할 내용
...
// // 1번 인덱스 에서 3번 인덱스 전까지의 데이터를 가져오기
...
// document.write("<br>split - " + ( str.split("-") ));
...
let total = 0; // 전체 키의 합
let talls = new Array(); // 키 저장 배열
let avg_below = new Array(); // 평균키보다 작은 사람들이 누구인지



window.onload=function(){
    $("#names").text( member[idx] );
    $("#talls").focus();
}

function action(){
    talls.push( Number($("#talls").val()) );
    total += talls[idx];
    if(idx == member.length-1){
        ...
        out += "</ul>";
        out += "평균 키 : "+avg+"cm";
        out += "<div>평균 보다 작은 사람 : "+avg_below+"</div>";
        $("#result").html( out );
    }
    $("#names").text( member[++idx] );
    $("#talls").val("");
